...
contract KittyVault {
    ...
    address public immutable i_token; //collateral
    address public immutable i_pool;
    AggregatorV3Interface public immutable i_priceFeed;
    AggregatorV3Interface public immutable i_euroPriceFeed;
    address public meowntainer; //maintains!
    IAavePool public immutable i_aavePool; //taking collateral here
    ...
    function executeDepawsit(
        ...
        if (_totalMeowllateral == 0) {
            ...
            //q mulDiv takes 3 params. why 2?
            _cattyNipGenerated = _ameownt.mulDiv(
                totalCattyNip,
                _totalMeowllateral
            );
        }
     ...
     */
    function purrrCollateralToAave(
        uint256 _ameowntToSupply
     ...
     */
    function purrrCollateralFromAave(
        uint256 _ameowntToWhiskdraw
    ) external onlyMeowntainer {
        totalMeowllateralInVault += _ameowntToWhiskdraw;
        i_aavePool.withdraw({
            asset: i_token,
            amount: _ameowntToWhiskdraw,
            to: address(this)
        });
    }
    ...
    function getUserMeowllateral(address _user) public view returns (uint256) {
        ...
        return
            userToCattyNip[_user].mulDiv(
                totalMeowllateralOfVault,
                totalCattyNip
            );
    }
